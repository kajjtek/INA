...
ExperimentResult run_single_experiment(int n, bool ascending_insert) {
    ...
    for (int run = 0; run < NUM_RUNS; ++run) {
        RBTree tree; // A new tree for each run
        ...
        for (int key : insert_keys) {
            RBTree::reset_counters(); // Reset counters for current operation
            tree.insert(key);
            all_insert_comparisons.push_back(RBTree::key_comparisons);
            all_insert_pointer_ops.push_back(RBTree::pointer_operations);
            all_insert_heights.push_back(tree.height()); // tree.height() updates RBTree::current_height
        }
        ...
        for (int key : delete_keys) {
            RBTree::reset_counters(); // Reset counters for current operation
            tree.deletion(key);
            all_delete_comparisons.push_back(RBTree::key_comparisons);
            all_delete_pointer_ops.push_back(RBTree::pointer_operations);
            all_delete_heights.push_back(tree.height()); // tree.height() updates RBTree::current_height
        }
    }
