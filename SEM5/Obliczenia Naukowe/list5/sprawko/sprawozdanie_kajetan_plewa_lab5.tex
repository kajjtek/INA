...
\begin{document}
	...
	\end{lstlisting}
	Podstawowa struktura \textit{block} składa się z macierzy $l x l$ - początkowo zarówno dla B jak i C miał być używany vektor - jednak dwa poniższe czynniki wymuszają użycie macierzy w obu przypadkach:
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item Zerowanie współczynnik pod przekątna znajdujących się w bloku $B$. Z charakterystycznej konstrukcji możemy zrobić to tylko raz gdyż współczynniki znajdują się tylko w pierwszym wierszu i ostatniej kolumnie
	\end{itemize}
	...
	Jest on oparty całkowicie na eliminacji Gaussa. Najbardziej istotną różnicą jest to, że w funkcji /textit{eliminateLU!} w przeciwieństwie do funkcji \textit{eliminate!} nie ustawiamy współczynnika $a_{ik}$ na $0$ tylko na obliczony iloraz $\frac{a_{ik}}{pivot}$. W ten sposób pod przekątną znajduje się macierz  $L$. \newline
