...
\begin{document}
	...
	\end{itemize}
	Status modelu jest \textbf{Optymalny}. Optymalne rozwiązanie wyznacza alokacje czasu maszyn, które maksymalizują zysk przy jednoczesnym spełnieniu wszystkich minimalnych wymagań produkcyjnych i ograniczeń czasowych maszyn.
	...
	\section{Zadanie 4b: Analiza Wyników Własnego Egzemplarza}
	...
	\subsection{Kryteria Egzemplarza i Wnioski}
	\begin{enumerate}
	...
	\subsection{Analiza Przypadków Optymalizacji}
	
	\subsubsection{Przypadek 1: Minimalizacja Kosztu z Limitem Czasowym $T = 15$}
...
\begin{center}
	\begin{tikzpicture}[
		>=Stealth,
		node distance=2.5cm,
		vertex/.style={circle, draw=black, fill=blue!10, minimum size=6mm},
		highlight/.style={circle, draw=black, fill=red!30, minimum size=6mm, ultra thick},
		pathA/.style={->, very thick, blue, densely dashed}, % Najtańsza bez ograniczeń (3.0, czas 20)
		pathB/.style={->, very thick, red, line width=1.5pt}, % Optymalna z ograniczeniem (5.0, czas 15)
		pathC/.style={->, thick, draw=gray, dotted}, % Nieosiągalne lub droższe ścieżki
		edge_style/.style={->, thick, draw=black!70}
		]
		
		% Wierzchołki (Nodes)
		\node (v1) [highlight, label=left:{\textbf{START}}] {1};
		\node (v2) [vertex, right=of v1] {2};
		\node (v3) [vertex, right=of v2] {3};
		\node (v5) [vertex, below right=1.5cm and 0.5cm of v2] {5};
		\node (v6) [vertex, below=of v5] {6};
		\node (v10) [highlight, right=of v3, label=right:{\textbf{KONIEC}}] {10};
		
		% ---------------------------------
		% SCIEZKA OPTYMALNA Z OGRANICZENIEM (KOSZT 5, CZAS 15) - 3 krawędzie
		% ---------------------------------
		\draw[pathB] (v1) -- (v2) node [midway, above] {\textbf{2}/5};
		\draw[pathB] (v2) -- (v3) node [midway, above] {\textbf{1}/5};
		\draw[pathB] (v3) -- (v10) node [midway, above] {\textbf{2}/5};
		
		% ---------------------------------
		% NAJTAŃSZA ŚCIEŻKA BEZ OGRANICZEŃ (KOSZT 3, CZAS 20) - 2 krawędzie
		% ---------------------------------
		\draw[pathA] (v1) to (v5) node [midway, below left, sloped] {1/10};
		\draw[pathA] (v5) to (v10) node [midway, below right, sloped] {2/10};
		
		% ---------------------------------
		% INNE KRAWEDZIE (KONTROLA)
		% ---------------------------------
		% Scieżka 1-6-10 (Koszt 6, Czas 15) - Feasible, ale droższa niż P_B
		\draw[pathC] (v1) to (v6) node [midway, left] {3/10};
		\draw[pathC] (v6) to (v10) node [midway, below] {3/5};
		
		% Dodatkowe krawędzie
		\draw[edge_style] (v2) -- (v5) node [midway, left] {1/5};
		\draw[edge_style] (v3) -- (v5) node [midway, right, sloped] {1/5};
		
		% ---------------------------------
		% Legenda - Poprawiona struktura
		\node [below right=3cm and 0.5cm of v1, align=left] (legend) {
			\textbf{Ścieżki optymalne}\\[0.1cm]
			\textcolor{red}{\textbf{\rule{1cm}{1.5pt}}} \textbf{Z Ograniczeniem Czasowym} ($C=5.0, T=15$)\\[0.1cm]
			\textcolor{blue}{\textbf{\rule{1cm}{1.5pt} \small -- -- --}} \textbf{Bez Ograniczenia Czasowego} ($C=3.0, T=20$)\\[0.1cm]
			\textcolor{gray}{\textbf{\rule{1cm}{1.5pt} \small $\cdots$}} Inne ścieżki Feasible (np. $1 \to 6 \to 10$, $C=6.0, T=15$).
		};
...
\subsubsection{(c) Całkowitoliczbowość zmiennych decyzyjnych}
 Założenie o całkowitoliczbowości w tym przypadku jest potrzebne, ponieważ bez niego optymalizator może wybierać niektóre krawędzie częściowo i
dzięki temu obniżać sztucznie całkowity koszt. Na przykładzie z poprzedniego
podpunktu bez założenia całkowitoliczbowości algorytm wybrał krawędź 2ß10,
z tym że wybrał ją z wagą 0.25, ponieważ normalnie nie mógłby jej użyć ze
względu na zbyt duży koszt.


\subsubsection{(d) Akceptowalność rozwiązania po usunięciu ograniczenia czasowego}
 Bez ograniczenia czasowego optymalizator zwróci poprawne wyniki, ponieważ wtedy wybieranie zależy tylko od czasu i ze względu na warunek równych
...
\begin{itemize}
	\item Ustala minimalną siłę roboczą na 48 osób.
	...
	\item Dostarcza konkretny plan alokacji: np. Zmiana 2 otrzymuje największą liczbę pracowników ($\mathbf{20}$), a Dzielnica 3 dostarcza najwięcej pracowników ($\mathbf{23}$).
\end{itemize}
...
\textbf{Weryfikacja Kontradykcji:}
Dla $k=3$ na siatce $5 \times 5$ zasięg każdej kamery jest tak duży, że dwie kamery (np. umieszczone w rogach, $(1, 1)$ i $(5, 5)$) powinny wystarczyć do pokrycia całej siatki. Fakt, że model wymaga $\mathbf{5}$ kamer, sugeruje, że model może zawierać \textbf{dodatkowe, nieujawnione ograniczenia} geometryczne (np. związane ze specyficznym ułożeniem, które preferuje ten model, lub błąd w logice pokrycia dla bardzo dużego $k$). Przy tak dużym zasięgu, $\mathbf{5}$ jest rozwiązaniem *akceptowalnym*, ale \textbf{prawdopodobnie nie optymalnym} w sensie matematycznym dla problemu minimalnego pokrycia zbiorem z tak szeroką maską.
...
\begin{itemize}
	\item Dla $k=2$ zasięg jest mniejszy, a $\mathbf{5}$ kamer wydaje się być bardziej realistyczną, choć nadal prawdopodobnie nie minimalną wartością dla tego typu maski.
	\item Fakt, że zmiana $k$ (znaczne zwiększenie zasięgu) nie wpłynęła na minimalną liczbę kamer, silnie sugeruje, że \textbf{struktura maski krzyżowej} (pokrywającej tylko wiersz i kolumnę, a nie cały kwadrat) jest dominującym czynnikiem wymuszającym dużą liczbę kamer, co jest sprzeczne z opisem "Zasięg każdej kamery (kwadrat...)" w wynikach.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Niezgodność Opisu:} Istnieje niezgodność pomiędzy opisem zasięgu w wynikach ("Kwadrat o boku X") a implementacją modelu (`scan\_squares`), która wyraźnie tworzy \textbf{maskę krzyżową}. To ograniczenie na zasięg (tylko wiersze i kolumny) wymaga większej liczby kamer, co wyjaśnia, dlaczego model nie znalazł rozwiązania z 2 lub 3 kamerami, nawet przy dużym promieniu $k=3$.
\end{itemize}
