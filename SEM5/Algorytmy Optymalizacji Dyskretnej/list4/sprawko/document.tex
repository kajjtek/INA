...
\begin{document}
	...
	\begin{itemize}
		...
		\item \textbf{Budowa hiperkostki:} Wierzchołki są identyfikowane przez liczby od $0$ do $2^k-1$. Krawędź $(i, j)$ tworzona jest wtedy i tylko wtedy, gdy reprezentacje binarne $i$ oraz $j$ różnią się na dokładnie jednej pozycji, a waga Hamminga $H(j) = H(i) + 1$.
		\item \textbf{Pojemności krawędzi:} Zgodnie z treścią zadania, pojemności są losowane z rozkładu jednostajnego ze zbioru $\{1, \dots, 2^l\}$, gdzie $l = \max(H(i), Z(i), H(j), Z(j))$, a $Z(x)$ to liczba zer w zapisie binarnym.
	\end{itemize}
