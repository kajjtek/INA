...
#include "../include/RadixDijkstra.h" // Zmieniono na RadixDijkstra.h
#include <iostream>
#include <fstream>
#include <chrono>
#include <cmath>
...
// Funkcja pomocnicza do wyświetlania użycia programu (z nowym parametrem -c)
void printUsage(const std::string& appName) {
    std::cerr << "Uzycie: " << appName << " -d <plik_grafu.gr> -c <max_cost> [-ss <plik_zrodel.ss>] [-oss <plik_wynikow_ss.res>] [-p2p <plik_par.p2p>] [-op2p <plik_wynikow_p2p.res>]\n";
    std::cerr << "Wymagane:\n";
    std::cerr << "  -d <plik_grafu.gr>     Sciezka do pliku grafu w formacie DIMACS.\n";
    std::cerr << "  -c <max_cost>          Maksymalna waga krawedzi (wymagane dla algorytmu Radix Heap).\n";
...
void saveSSSPDistances(
    ...
    outFile << "c Wyniki dla algorytmu Radix Heap Dijkstry, zrodlo: " << sourceNode << "\n";
...
int main(int argc, char* argv[]) {
    ...
    if (maxCost == -1) {
        std::cerr << "Blad: Wymagana maksymalna waga krawedzi (-c) dla algorytmu Radix Heap.\n";
        printUsage(argv[0]);
        return 1;
    }
